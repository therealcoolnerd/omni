...
impl DockerClient {
    ...
    pub async fn create_container(&self, config: &DockerConfig) -> Result<String> {
        info!("Creating container from image: {}:{}", config.image, config.tag);
        ...
        if config.security_options.no_new_privileges {
            args.extend(vec!["--security-opt".to_string(), "no-new-privileges".to_string()]);
        }
        ...
        for volume in &config.volumes {
            ...
            args.extend(vec![
                "-v".to_string(),
                format!("{}:{}:{}", volume.host_path.display(), volume.container_path, mode)
            ]);
        }
        ...
        if output.status.success() {
            ...
            info!("Container created successfully: {} ({})", container_name, container_id);
            Ok(container_id)
        } else {
    ...
    pub async fn install_package_in_container(
        ...
        self.execute_command(container_id, &command, Some("root")).await
    }
    ...
    pub async fn copy_to_container(
        ...
        info!("Copying file to container: {} -> {}", local_path.display(), container_path);
    ...
    pub async fn copy_from_container(
        ...
        info!("Copying file from container: {} -> {}", container_path, local_path.display());
    ...
    fn parse_container_inspect_json(&self, json: &serde_json::Value) -> Option<ContainerInfo> {
        let id = json.get("Id")?.as_str()?.to_string();
        let name = json.get("Name")?.as_str()?.trim_start_matches('/').to_string();
...
impl DockerPackageManager {
    ...
    pub async fn install_package_isolated(
        ...
        let image = self.base_images.get(package_manager)
        ...
        // Install package
        let result = self.client.install_package_in_container(
            &container_id,
            package_manager,
            package_name
        ).await;
        ...
        for (pm, _) in &self.base_images {
            ...
            match self.install_package_isolated(pm, package_name).await {
                ...
                Err(e) => {
                    ...
                    results.insert(pm.clone(), DockerCommandResult {
