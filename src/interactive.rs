...
use crate::security::{TrustLevel, VerificationResult};
use anyhow::Result;
...
impl InteractivePrompts {
    ...
    pub fn confirm_installation(&self, plan: &ResolutionPlan) -> Result<InstallConfirmation> {
        ...
        for (i, package) in plan.packages.iter().enumerate() {
            ...
            println!(
                "{} {} [{}] v{}",
                status, package.name, package.box_type, package.version
            );
        }
        ...
        let proceed = if !plan.conflicts.is_empty() {
            ...
            match selection {
                0 => false,
                1 => Confirm::with_theme(&self.theme)
                    .with_prompt(
                        "⚠️  Are you sure you want to force installation despite conflicts?",
                    )
                    .default(false)
                    .interact()?,
                2 => {
        ...
        let optional_deps: Vec<&ResolvedPackage> = plan
            .packages
            .iter()
            .filter(|p| p.dependencies.iter().any(|d| d.optional))
            .collect();

        let mut selected_packages: Vec<String> = plan
            .packages
            .iter()
            .filter(|p| !p.dependencies.iter().any(|d| d.optional))
            .map(|p| p.name.clone())
    ...
    pub fn select_from_search_results(
        &self,
    ...
    pub fn display_error_with_options(
        &self,
        error: &anyhow::Error,
        recoverable: bool,
    ) -> Result<bool> {
...
impl OmniError {
    ...
    pub fn suggested_action(&self) -> &'static str {
        match self {
            ...
            OmniError::DependencyResolutionFailed { .. } => {
                "Check for conflicting packages or try manual installation"
            }
            OmniError::SecurityVerificationFailed { .. } => {
                "Verify package source or use --allow-untrusted flag"
            }
