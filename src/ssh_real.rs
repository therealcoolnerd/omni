...
impl RealSshSession {
    ...
    pub async fn execute_command(&mut self, command: &str) -> Result<RealSshCommandResult> {
        ...
        let session = self.client_session.as_ref()
        ...
        match result {
            Ok((exit_code, stdout, stderr)) => {
                ...
                if result.success() {
                    ...
                    warn!("Command failed with exit code {}: {}", exit_code, result.stderr);
                }
    ...
    pub async fn execute_commands(&mut self, commands: &[&str]) -> Result<Vec<RealSshCommandResult>> {
    ...
    pub async fn execute_privileged_command(&mut self, command: &str, sudo_password: Option<&str>) -> Result<RealSshCommandResult> {
    ...
    pub async fn upload_file(&mut self, local_path: &PathBuf, remote_path: &str) -> Result<()> {
        ...
        info!("Uploading file: {} -> {}", local_path.display(), remote_path);
        ...
        // Create the file
        self.execute_command(&format!("touch {}", remote_path)).await?;
        
        // Upload in chunks
        for (i, chunk) in encoded.chars().collect::<Vec<_>>().chunks(chunk_size).enumerate() {
            ...
            if !result.success() {
                return Err(anyhow!("File upload failed at chunk {}: {}", i, result.stderr));
            }
        }
    ...
    pub async fn download_file(&mut self, remote_path: &str, local_path: &PathBuf) -> Result<()> {
        ...
        info!("Downloading file: {} -> {}", remote_path, local_path.display());
        ...
        // Decode and save
        let content = base64::prelude::BASE64_STANDARD.decode(result.stdout.trim())
    ...
    pub async fn disconnect(&mut self) -> Result<()> {
        if self.connected {
            ...
            if let Some(session) = &self.client_session {
                let session_guard = session.lock().await;
                session_guard.disconnect(Disconnect::ByApplication, "", "en-US").await?;
            }
    ...
    async fn authenticate(&self, session: &client::Handle<SshClientHandler>) -> Result<()> {
        ...
        match &self.config.auth_method {
            ...
            RealAuthMethod::PublicKey { private_key_path, passphrase } => {
                // Load private key
        ...
        if self.connected {
            ...
            if let Some(session) = self.client_session.clone() {
                tokio::spawn(async move {
                    let session_guard = session.lock().await;
                    let _ = session_guard.disconnect(Disconnect::ByApplication, "", "en-US").await;
                });
            }
        }
    }
...
impl RealSshConnectionPool {
    ...
    pub async fn get_or_create_session(&mut self, host: &str, config: RealSshConfig) -> Result<&mut RealSshSession> {
...
impl RealSshClient {
    ...
    pub async fn test_host_connectivity(&mut self, host: &str, config: RealSshConfig) -> Result<bool> {
    ...
    pub async fn get_multi_host_info(&mut self, hosts: Vec<(String, RealSshConfig)>) -> Result<HashMap<String, SystemInfo>> {
