...
impl SecurityVerifier {
    ...
    pub async fn verify_package(
        ...
        info!("Starting security verification for package: {:?}", package_path);
        ...
        if self.policy.verify_checksums {
            if let Some(expected) = expected_checksum {
                ...
                if result.checksum_valid == Some(false) {
                    result.warnings.push("Checksum verification failed".to_string());
                    return Ok(result);
                }
            } else {
                result.warnings.push("No checksum provided for verification".to_string());
            }
        }
        ...
        if self.policy.verify_signatures {
            if let Some(sig_url) = signature_url {
                ...
                if let Some(found_sig) = self.find_signature_file(package_path, box_type).await? {
                    result.signature_valid = Some(self.verify_signature(package_path, &found_sig).await?);
                }
            }
        }
        ...
        self.perform_package_specific_checks(package_path, box_type, &mut result).await?;
        
        result.details = self.generate_verification_summary(&result);
        
        info!("Security verification completed with trust level: {:?}", result.trust_level);
        Ok(result)
    }
        ...
        match output {
            ...
            Err(e) => {
                ...
                if self.policy.allow_untrusted {
                    ...
                    Err(anyhow::anyhow!("GPG verification unavailable and untrusted packages not allowed"))
                }
            }
        }
    }
    ...
    async fn find_signature_file(&self, package_path: &Path, box_type: &str) -> Result<Option<String>> {
        // Common signature file patterns
        let base_name = package_path.file_name().unwrap_or_default().to_string_lossy();
    ...
    async fn verify_appimage_security(
        ...
        #[cfg(unix)]
        {
            ...
            if permissions.mode() & 0o111 == 0 {
                result.warnings.push("AppImage is not executable".to_string());
            }
        }
        ...
        if let Ok(output) = output {
            ...
            if !file_type.contains("ELF") {
                result.warnings.push("AppImage does not appear to be a valid ELF executable".to_string());
            }
        }
    ...
    async fn verify_snap_security(
        ...
        result.details.push_str("Snap packages are verified by the Snap Store. ");
        Ok(())
    }
    ...
    async fn verify_flatpak_security(
        ...
        result.details.push_str("Flatpak packages are sandboxed and verified by repositories. ");
        Ok(())
    }
        ...
        if let Ok(output) = output {
            ...
            if file_type.contains("script") || file_type.contains("executable") {
                if !file_type.contains("Debian") && !file_type.contains("RPM") {
                    result.warnings.push("Package contains executable content outside of standard package format".to_string());
                }
            }
        }
    ...
    pub async fn verify_repository_metadata(&self, box_type: &str) -> Result<VerificationResult> {
        ...
        match box_type {
            ...
            _ => {
                result.details = format!("Repository verification not implemented for {}", box_type);
            }
        }
