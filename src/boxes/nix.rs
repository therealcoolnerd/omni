    ...
    fn install(&self, package: &str) -> Result<()> {
        tokio::runtime::Runtime::new()?.block_on(async {
            ...
            if result.exit_code == 0 {
                ...
                let result2 = self.executor.execute_package_command(
                    "nix-env",
                    &["-i", package],
                    config2
                ).await?;
                ...
                if result2.exit_code == 0 {
                    info!("✅ Nix successfully installed '{}' (fallback method)", package);
                    Ok(())
                } else {
                    error!("❌ Nix failed to install '{}' with both methods: {}", package, result2.stderr);
    ...
    fn update(&self, package: Option<&str>) -> Result<()> {
        tokio::runtime::Runtime::new()?.block_on(async {
            if let Some(pkg) = package {
                ...
                let _update_result = self.executor.execute_package_command(
                    "nix-channel",
                    &["--update"],
                    config.clone()
                ).await;
