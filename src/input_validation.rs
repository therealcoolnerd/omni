...
impl InputValidator {
    ...
    pub fn validate_box_type(box_type: &str) -> Result<()> {
        ...
        let valid_box_types = [
            "apt", "dnf", "pacman", "snap", "flatpak", "appimage",
            "winget", "chocolatey", "scoop", "homebrew", "mas"
        ];
...
mod tests {
    ...
    fn test_command_args_validation() {
        // Valid args
        assert!(InputValidator::validate_command_args(&["install".to_string(), "firefox".to_string()]).is_ok());
        assert!(InputValidator::validate_command_args(&["-y".to_string(), "--force".to_string()]).is_ok());
