...
impl SecureExecutor {
    ...
    pub async fn secure_install(
        ...
        info!("Starting secure installation of {} via {}", package_name, box_type);
        ...
        match box_type {
            "apt" => {
                self.execute_apt_install(package_name, config).await
            }
            "dnf" => {
                self.execute_dnf_install(package_name, config).await
            }
            "pacman" => {
                self.execute_pacman_install(package_name, config).await
            }
            "snap" => {
                self.execute_snap_install(package_name, config).await
            }
            "flatpak" => {
                self.execute_flatpak_install(package_name, config).await
            }
            _ => {
                Err(OmniError::UnsupportedBoxType {
    ...
    fn validate_flag(&self, flag: &str) -> Result<()> {
        ...
        let safe_flags = [
            ...
            "-h", "--help",
            "--version",
            "-R", "-S", "-Q", // pacman specific
            "--user", "--system", // flatpak specific
    ...
    async fn execute_apt_install(&self, package: &str, config: ExecutionConfig) -> Result<ExecutionResult> {
        // Update package lists first
        let update_result = self.execute_package_command(
            "apt",
            &["update"],
            ExecutionConfig { requires_sudo: true, ..config.clone() }
        ).await;
    ...
    async fn execute_dnf_install(&self, package: &str, config: ExecutionConfig) -> Result<ExecutionResult> {
        self.execute_package_command(
            "dnf",
            &["install", "-y", package],
            config
    ...
    async fn execute_pacman_install(&self, package: &str, config: ExecutionConfig) -> Result<ExecutionResult> {
        // Sync package databases first
        let sync_result = self.execute_package_command(
            "pacman",
            &["-Sy"],
            ExecutionConfig { requires_sudo: true, ..config.clone() }
        ).await;
    ...
    async fn execute_snap_install(&self, package: &str, config: ExecutionConfig) -> Result<ExecutionResult> {
        self.execute_package_command(
            "snap",
            &["install", package],
            config
    ...
    async fn execute_flatpak_install(&self, package: &str, config: ExecutionConfig) -> Result<ExecutionResult> {
        self.execute_package_command(
            "flatpak",
            &["install", "-y", package],
            config
