...
impl Sandbox {
    ...
    pub fn new() -> Result<Self> {
        let work_dir =
            TempDir::new().map_err(|e| anyhow!("Failed to create sandbox directory: {}", e))?;
    ...
    fn validate_path(&self, path: &Path) -> Result<()> {
        ...
        let forbidden_prefixes = ["/etc/", "/boot/", "/sys/", "/proc/", "/dev/", "/root/"];
    ...
    pub fn check_resources(&self) -> Result<ResourceUsage> {
        ...
        // Check limits
        if usage.disk_usage > 1024 * 1024 * 1024 {
            // 1GB limit
