...
use crate::boxes::{apt, dnf, pacman, flatpak, snap, appimage};
use crate::distro;
use anyhow::Result;
...
impl SnapshotManager {
    ...
    pub async fn create_snapshot(&self, name: &str, description: Option<&str>) -> Result<String> {
        ...
        info!("✅ Successfully created snapshot '{}' with ID: {}", name, snapshot_id);
        Ok(snapshot_id)
    }
    ...
    pub async fn revert_to_snapshot(&self, snapshot_id: &str) -> Result<()> {
        ...
        info!("✅ Successfully reverted to snapshot '{}'", target_snapshot.name);
        Ok(())
    }
    ...
    async fn install_package(&self, package: &InstallRecord) -> Result<()> {
        info!("Installing package {} via {}", package.package_name, package.box_type);
        ...
        match package.box_type.as_str() {
            ...
            "flatpak" if distro::command_exists("flatpak") => {
                let name = package.source_url.as_deref().unwrap_or(&package.package_name);
    ...
    async fn remove_package(&self, package: &InstallRecord) -> Result<()> {
        info!("Removing package {} via {}", package.package_name, package.box_type);
    ...
    pub async fn auto_snapshot(&self, operation: &str, package: &str) -> Result<Option<String>> {
        let snapshot_name = format!("auto-{}-{}-{}", operation, package, Utc::now().format("%Y%m%d-%H%M%S"));
        let description = Some(format!("Automatic snapshot before {} {}", operation, package));
        
        match self.create_snapshot(&snapshot_name, description.as_deref()).await {
            Ok(snapshot_id) => {
