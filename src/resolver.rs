...
impl DependencyResolver {
    ...
    pub async fn resolve_dependencies(
        ...
        while let Some((pkg_name, preferred_box, depth)) = queue.pop_front() {
            ...
            let dependencies = self.get_package_dependencies(&pkg_name, preferred_box.as_deref()).await?;
        ...
        info!("Dependency resolution complete. {} packages to install", resolution_plan.packages.len());
    ...
    async fn get_package_dependencies(
        ...
        if let Some(box_type) = box_type {
            if let Ok(Some(cached)) = self.db.get_cached_package_info(package_name, box_type).await {
                return Ok(cached.dependencies.into_iter().map(|name| Dependency {
                    name,
                    version_req: None,
        ...
        let dependencies = match box_type {
            ...
            _ => {
                ...
                self.get_dependencies_from_available_boxes(package_name).await
            }
        };
            ...
            if line.trim().starts_with("Depends:") {
                ...
                if !conflict_part.is_empty() {
                    ...
                    if let Some(last_dep) = dependencies.last_mut() {
                        last_dep.conflicts.push(conflict_part.split_whitespace().next().unwrap_or("").to_string());
                    }
                }
            }
        }
    ...
    async fn get_dependencies_from_available_boxes(&self, package_name: &str) -> Result<Vec<Dependency>> {
    ...
    async fn check_conflicts(&self, package_name: &str, dependencies: &[Dependency]) -> Result<Vec<String>> {
        let mut conflicts = Vec::new();
        
        // Get currently installed packages
        let installed = self.db.get_installed_packages().await?;
        let installed_names: HashSet<String> = installed.iter().map(|p| p.package_name.clone()).collect();
    ...
    async fn calculate_total_size(&self, packages: &[ResolvedPackage]) -> Option<u64> {
        ...
        for package in packages {
            if let Ok(size) = self.get_package_size(&package.name, &package.box_type).await {
                total_size += size;
                found_any_size = true;
            }
        }
